...
void stats::show(bool per_sec)
{
	...
	for (stats_map::const_iterator iter = discontinuities.begin(); iter != discontinuities.end(); ++iter)
		__log_printf(stderr, "pid %04x\t%" PRIu64 " continuity errors (%" PRIu64 "%%)", iter->first, iter->second, ((!iter->second) || (!statistics[iter->first])) ? 0 : (!statistics.count(iter->first)) ? 0 : (100 * iter->second / (statistics[iter->first] / 188)));

	if (tei_count) __log_printf(stderr, "tei count: %" PRIu64 " (%" PRIu64 "%%)", tei_count, (!statistics[0x2000]) ? 0 : (18800 * tei_count / statistics[0x2000]));
}
...
void stats::push(const uint8_t *p, pkt_stats_t *pkt_stats)
{
	...
	if (hdr.adaptation_flags & 0x02) {
		if (adapt.pcr) {
			...
			__log_printf(stderr, "PID: 0x%04x, PCR base: %" PRIu64 ", ext: %d", hdr.pid, pcr_base, pcr_ext);
